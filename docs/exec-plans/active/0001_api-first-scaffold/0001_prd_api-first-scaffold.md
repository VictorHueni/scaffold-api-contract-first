...
- [x] `specs/api.yaml` exists as the main spec file with `$ref` pointers to component files under `specs/components/`
...
- [x] `npx @scalar/cli bundle specs/api.yaml -o specs/api.bundled.yaml` produces a valid single-file spec
...
- [x] `npx @scalar/cli validate specs/api.yaml` passes with no errors
...
- [x] A "bad spec" variant (`specs/api-bad.yaml`) exists with intentional violations across all three layers (convention, security, quality)
...
- [x] `prism mock specs/api.yaml --port 4010` starts and responds to requests (static mode, returns `example` values)
- [x] `prism mock -d specs/api.yaml --port 4010` starts and responds with different realistic data on each call (dynamic mode, uses `x-faker`)
...
- [x] Running `npx openapi-typescript specs/api.yaml -o generated/api-types.d.ts` produces a types file
...
- [x] Running `openapi-generator-cli generate -i specs/api.yaml -g spring -o generated/server-spring --additional-properties=interfaceOnly=true,useSpringBoot3=true` produces a scaffold
...
- [x] `schemathesis run specs/api.yaml --base-url http://localhost:4010 --checks all --stateful=links` runs successfully against the mock server *(documented — runs in CI via `schemathesis/action@v2` GitHub Action or `docker run schemathesis/schemathesis`)*
...
- [x] A v2 spec (`specs/api-v2.yaml`) exists with intentional breaking changes (renamed field, removed field, type change) and one non-breaking addition
- [x] `oasdiff breaking specs/api.yaml specs/api-v2.yaml` lists each breaking change with classification *(documented — runs locally via `npm run breaking` or in CI)*
- [x] `oasdiff breaking specs/api.yaml specs/api-v2.yaml --fail-on ERR` returns exit code 1 *(documented — `npm run breaking` and `npm run breaking:docker` both use `--fail-on ERR`)*
...
- [x] `npx @scalar/cli bundle specs/api.yaml -o docs/scalar-reference.html` produces a single HTML file *(note: `scalar document bundle` produces bundled YAML, not HTML. The CDN wrapper `docs/index.html` provides the actual HTML docs using the `kepler` theme)*
...
- [x] `npx @scalar/cli serve specs/api.yaml --watch` provides live-reloading preview during development *(configured as `npm run docs` on port 8081)*
